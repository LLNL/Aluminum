...
public:
   ...
   * stream.
   */
  AlGpuStream_t get_high_priority_stream();
   ...
   */
  void replace_streams(std::function<AlGpuStream_t(bool)> stream_getter);

private:
  std::vector<AlGpuStream_t> default_streams;
  std::atomic<uint32_t> default_idx{0};
  std::vector<AlGpuStream_t> high_priority_streams;
