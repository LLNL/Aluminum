...
class SendAlState : public AlState {
 public:
  SendAlState(const T* sendbuf, size_t count_, int dest_,
              HostTransferCommunicator& comm_, AL_GPU_RT(Stream_t) stream) :
    ...
    AL_CHECK_CUDA(AL_GPU_RT(MemcpyAsync)(mem, sendbuf, sizeof(T)*count,
                                  AL_GPU_RT(MemcpyDeviceToHost), stream));
 ...
 private:
  ...
  AL_GPU_RT(Stream_t) compute_stream;
#ifdef AL_HAS_PROF
...
class RecvAlState : public AlState {
 public:
  RecvAlState(T* recvbuf, size_t count_, int src_,
    ...
    AL_CHECK_CUDA(AL_GPU_RT(MemcpyAsync)(recvbuf, mem, sizeof(T)*count,
                                  AL_GPU_RT(MemcpyHostToDevice), stream));
    sync_event.record(stream);
  }
...
class SendRecvAlState : public AlState {
 public:
  SendRecvAlState(const T* sendbuf, size_t send_count, int dest,
                  ...
                  HostTransferCommunicator& comm, AL_GPU_RT(Stream_t) stream) :
    AlState(nullptr),
