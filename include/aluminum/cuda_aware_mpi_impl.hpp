...
#include "cuda_aware_mpi/allgather.hpp"
#include "cuda_aware_mpi/allreduce.hpp"
#include "cuda_aware_mpi/alltoall.hpp"
#include "cuda_aware_mpi/bcast.hpp"
#include "cuda_aware_mpi/gather.hpp"
#include "cuda_aware_mpi/reduce.hpp"
#include "cuda_aware_mpi/reduce_scatter.hpp"
#include "cuda_aware_mpi/scatter.hpp"
#include "cuda_aware_mpi/pt2pt.hpp"

namespace Al {

enum class CUDAAwareMPICollectiveAlgorithm {
  automatic
};

inline std::string algorithm_name(CUDAAwareMPICollectiveAlgorithm algo) {
  switch (algo) {
  case CUDAAwareMPICollectiveAlgorithm::automatic:
    return "automatic";
  default:
    return "unknown";
  }
...
/** Initialize CUDA-aware MPI backend. */
void init(int& argc, char**& argv);
/** Finalize CUDA-aware MPI backend. */
void finalize();

/** Represents a request for the CUDA-aware MPI backend. */
struct CUDAAwareMPIRequest {
  CUDAAwareMPIRequest(cudaEvent_t op_event_, cudaStream_t orig_stream_) :
    op_event(op_event_), orig_stream(orig_stream_) {}
  // Note: Not thread safe!
  ~CUDAAwareMPIRequest() { cuda::release_cuda_event(op_event); }
...
class CUDAAwareMPIBackend {
  friend void internal::cuda_aware_mpi::init(int&, char**&);
  friend void internal::cuda_aware_mpi::finalize();
 public:
  using allreduce_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using allgather_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using alltoall_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using bcast_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using gather_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using reduce_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using reduce_scatter_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using scatter_algo_type = CUDAAwareMPICollectiveAlgorithm;
  using comm_type = internal::cuda_aware_mpi::CUDAAwareMPICommunicator;
  using req_type = std::shared_ptr<internal::cuda_aware_mpi::CUDAAwareMPIRequest>;
  ...
  static void Allgather(
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIAllgather: Invalid algorithm");
    }
  }
  ...
  static void NonblockingAllgather(
    ...
    comm_type& comm, req_type& req, allgather_algo_type algo) {
    NonblockingAllgather<T>(internal::IN_PLACE<T>(), buffer, count, comm, req, algo);
  }
  ...
  static void Alltoall(
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIAlltoall: Invalid algorithm");
    }
  }
  ...
  static void NonblockingAlltoall(
    T* buffer, size_t count,
    comm_type& comm, req_type& req, alltoall_algo_type algo) {
    NonblockingAlltoall<T>(internal::IN_PLACE<T>(), buffer, count, comm, req, algo);
  }
  ...
  static void Bcast(T* buf, size_t count, int root, comm_type& comm,
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIBcast: Invalid algorithm");
    }
  }
  ...
  static void Gather(
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIGather: Invalid algorithm");
    }
  }
  ...
  static void NonblockingGather(
    ...
    comm_type& comm, req_type& req, gather_algo_type algo) {
    NonblockingGather<T>(internal::IN_PLACE<T>(), buffer, count, root, comm, req, algo);
  }
  ...
  static void Reduce(
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIReduce: Invalid algorithm");
    }
  }
  ...
  static void NonblockingReduce(
    ...
    comm_type& comm, req_type& req, reduce_algo_type algo) {
    NonblockingReduce<T>(internal::IN_PLACE<T>(), buffer, count, op, root, comm, req, algo);
  }
  ...
  static void Reduce_scatter(
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIReduce_scatter: Invalid algorithm");
    }
  }
  ...
  static void Scatter(
    ...
    case CUDAAwareMPICollectiveAlgorithm::automatic:
      ...
      break;
    default:
      throw_al_exception("CUDAAwareMPIScatter: Invalid algorithm");
    }
  }
  ...
  static void Scatter(
    ...
    Scatter(buffer, internal::IN_PLACE<T>(), count, root, comm, algo);
  }
  ...
  static void NonblockingScatter(
    T* buffer, size_t count, int root,
    comm_type& comm, req_type& req, scatter_algo_type algo) {
    NonblockingScatter<T>(buffer, internal::IN_PLACE<T>(), count, root, comm, req, algo);
  }

  static std::string Name() { return "CUDAAwareMPIBackend"; }
 ...
 private:
  ...
  static void setup_completion_event(
    ...
    req = std::make_shared<internal::cuda_aware_mpi::CUDAAwareMPIRequest>(event, comm.get_stream());
  }
  ...
  static void do_allreduce(
    ...
    internal::cuda_aware_mpi::AllreduceState<T>* state =
      new internal::cuda_aware_mpi::AllreduceState<T>(
  ...
  static void do_send(
    ...
    cudaStream_t stream) {
    internal::cuda_aware_mpi::SendState<T>* state =
      new internal::cuda_aware_mpi::SendState<T>(
  ...
  static void do_recv(
    ...
    cudaStream_t stream) {
    internal::cuda_aware_mpi::RecvState<T>* state =
      new internal::cuda_aware_mpi::RecvState<T>(
  ...
  static void do_sendrecv(
    ...
    T* recvbuf, size_t recv_count, int src, comm_type& comm,
    cudaStream_t stream) {
    internal::cuda_aware_mpi::SendRecvState<T>* state =
      new internal::cuda_aware_mpi::SendRecvState<T>(
  ...
  static void do_allgather(
    ...
    cudaStream_t stream) {
    internal::cuda_aware_mpi::AllgatherState<T>* state =
      new internal::cuda_aware_mpi::AllgatherState<T>(
  ...
  static void do_alltoall(
    const T* sendbuf, T* recvbuf, size_t count, comm_type& comm,
    cudaStream_t stream) {
    internal::cuda_aware_mpi::AlltoallState<T>* state =
      new internal::cuda_aware_mpi::AlltoallState<T>(
        sendbuf, recvbuf, count, comm, stream);
    internal::get_progress_engine()->enqueue(state);
  }
  ...
  static void do_bcast(
    ...
    cudaStream_t stream) {
    internal::cuda_aware_mpi::BcastState<T>* state =
      new internal::cuda_aware_mpi::BcastState<T>(
  ...
  static void do_gather(
    ...
    cudaStream_t stream) {
    internal::cuda_aware_mpi::GatherState<T>* state =
      new internal::cuda_aware_mpi::GatherState<T>(
  ...
  static void do_reduce(
    ...
    cudaStream_t stream) {
    internal::cuda_aware_mpi::ReduceState<T>* state =
      new internal::cuda_aware_mpi::ReduceState<T>(
  ...
  static void do_reduce_scatter(
    const T* sendbuf, T* recvbuf, size_t count, ReductionOperator op,
    comm_type& comm, cudaStream_t stream) {
    internal::cuda_aware_mpi::ReduceScatterState<T>* state =
      new internal::cuda_aware_mpi::ReduceScatterState<T>(
        sendbuf, recvbuf, count, op, comm, stream);
    internal::get_progress_engine()->enqueue(state);
  }
  ...
  static void do_scatter(
    const T* sendbuf, T* recvbuf, size_t count, int root, comm_type& comm,
    cudaStream_t stream) {
    internal::cuda_aware_mpi::ScatterState<T>* state =
      new internal::cuda_aware_mpi::ScatterState<T>(
        sendbuf, recvbuf, count, root, comm, stream);
    internal::get_progress_engine()->enqueue(state);
  }
...
template <>
inline bool Test<CUDAAwareMPIBackend>(typename CUDAAwareMPIBackend::req_type& req) {
  if (req == CUDAAwareMPIBackend::null_req) {
    return true;
  }
  // This is purely a host operation.
  bool r = cudaEventQuery(req->op_event) == cudaSuccess;
  if (r) {
    req = CUDAAwareMPIBackend::null_req;
  }
  return r;
}

template <>
inline void Wait<CUDAAwareMPIBackend>(typename CUDAAwareMPIBackend::req_type& req) {
  if (req == CUDAAwareMPIBackend::null_req) {
    return;
  }
