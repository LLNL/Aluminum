...
void time_reduce_algo(typename VectorType<Backend>::type input,
                      typename Backend::comm_type& comm,
                      typename Backend::reduce_algo_type algo,
                      CollectiveProfile<Backend, typename Backend::reduce_algo_type>& prof) {
  ...
  for (size_t trial = 0; trial < num_trials + 1; ++trial) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    start_timer<Backend>(comm);
    Al::Reduce<Backend>(in_place_input.data(), input.size(),
                        Al::ReductionOperator::sum, 0, comm, algo);
    if (trial > 0) {  // Skip warmup.
...
void do_benchmark(const std::vector<size_t> &sizes) {
  std::vector<typename Backend::reduce_algo_type> algos
      = get_reduce_algorithms<Backend>();
  typename Backend::comm_type comm;  // Use COMM_WORLD.
  CollectiveProfile<Backend, typename Backend::reduce_algo_type> prof("reduce");
  for (const auto& size : sizes) {
    auto data = gen_data<Backend>(size);
    // Benchmark algorithms.
    for (auto&& algo : algos) {
      MPI_Barrier(MPI_COMM_WORLD);
      time_reduce_algo<Backend>(data, comm, algo, prof);
    }
  }
  if (comm.rank() == 0) {
...
int main(int argc, char** argv) {
  ...
  if (backend == "MPI") {
    ...
    std::cout << "Reduce not supported on MPI-CUDA backend." << std::endl;
    std::abort();
#endif    
#ifdef AL_HAS_HOST_TRANSFER
  } else if (backend == "HT") {
