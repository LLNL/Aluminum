...
void time_bcast_algo(typename VectorType<Backend>::type input,
                     typename Backend::comm_type& comm,
                     typename Backend::bcast_algo_type algo,
                     CollectiveProfile<Backend, typename Backend::bcast_algo_type>& prof) {
  for (size_t trial = 0; trial < num_trials + 1; ++trial) {
    MPI_Barrier(MPI_COMM_WORLD);
    start_timer<Backend>(comm);
    Al::Bcast<Backend>(input.data(), input.size(), 0, comm, algo);
...
void do_benchmark(const std::vector<size_t> &sizes) {
  std::vector<typename Backend::bcast_algo_type> algos
      = get_bcast_algorithms<Backend>();
  typename Backend::comm_type comm;  // Use COMM_WORLD.
  CollectiveProfile<Backend, typename Backend::bcast_algo_type> prof("bcast");
  for (const auto& size : sizes) {
    auto data = gen_data<Backend>(size);
    // Benchmark algorithms.
    for (auto&& algo : algos) {
      MPI_Barrier(MPI_COMM_WORLD);
      time_bcast_algo<Backend>(data, comm, algo, prof);
    }
  }
  if (comm.rank() == 0) {
...
int main(int argc, char** argv) {
  ...
  if (backend == "MPI") {
    ...
    std::cout << "Bcast not supported on MPI-CUDA backend." << std::endl;
    std::abort();
#endif    
#ifdef AL_HAS_HOST_TRANSFER
  } else if (backend == "HT") {
