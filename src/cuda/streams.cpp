...
void StreamPool::allocate(size_t num_streams) {
  ...
  AL_CHECK_CUDA(AlGpuDeviceGetStreamPriorityRange(
                  &least_priority, &greatest_priority));
  default_streams.resize(num_streams);
  high_priority_streams.resize(num_streams);
  for (size_t i = 0; i < num_streams; ++i) {
    AL_CHECK_CUDA(AlGpuStreamCreate(&default_streams[i]));
    ...
    AL_CHECK_CUDA(AlGpuStreamCreateWithPriority(
          &high_priority_streams[i], AlGpuStreamDefault, greatest_priority));
...
void StreamPool::clear() {
  if (!external_streams) {
    for (size_t i = 0; i < default_streams.size(); ++i) {
      AL_CHECK_CUDA(AlGpuStreamDestroy(default_streams[i]));
      AL_CHECK_CUDA(AlGpuStreamDestroy(high_priority_streams[i]));
    }
  }
...
AlGpuStream_t StreamPool::get_stream() {
#ifdef AL_DEBUG
...
AlGpuStream_t StreamPool::get_high_priority_stream() {
#ifdef AL_DEBUG
...
void StreamPool::replace_streams(std::function<AlGpuStream_t(bool)> stream_getter) {
